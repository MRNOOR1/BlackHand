...
void alarm_service_init(void)
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        for (size_t j = s_count; j > 0; j--)
            s_alarms[j] = s_alarms[j - 1];
        s_alarms[0] = a;
        s_count++;
    }
...
Alarm *alarm_service_create(int hour, int minute, const char *label)
{
    ...
    char id[128];
    if (generate_alarm_id(id, sizeof(id)) != 0)
        return NULL;
...
void alarm_service_tick(void)
{
    ...
    static int last_fired_hour = -1;
    static int last_fired_min  = -1;

    if (tm_now.tm_hour == last_fired_hour && tm_now.tm_min == last_fired_min)
        return;

    last_fired_hour = tm_now.tm_hour;
    last_fired_min  = tm_now.tm_min;
    ...
    for (size_t i = 0; i < s_count; i++)
    {
        ...
        if (!a || !a->enabled)
            continue;
        if (a->hour != tm_now.tm_hour || a->minute != tm_now.tm_min)
            continue;

        /* tm_wday: 0=Sun, 6=Sat */
        if (a->repeat == ALARM_WEEKDAYS && (tm_now.tm_wday == 0 || tm_now.tm_wday == 6))
            continue;

        audio_ipc_play("/data/alarms/alarm.mp3");

        if (a->repeat == ALARM_ONCE)
        {
            a->enabled = false;
            write_alarm_file(a);
        }
