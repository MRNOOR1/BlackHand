...
int main(void) {
    ...
    while (1) {
        ...
        if (key == NCKEY_BUTTON1 || key == NCKEY_BUTTON2 || key == NCKEY_BUTTON3) {
            /*
             * Accept both PRESS (mouse click) and RELEASE (touchscreen lift).
             * Linux evdev touchscreens (HyperPixel 4.0 Goodix) typically only
             * report RELEASE — the finger-lift event.  Dropping RELEASE here
             * would make the entire touchscreen dead.
             *
             * Skip MOTION and other event types to avoid spurious triggers.
             */
            if (ni.evtype != NCTYPE_PRESS && ni.evtype != NCTYPE_RELEASE) {
                continue;
            }
         ...
         */
        if (current_screen != SCREEN_NOTES || key == '5') {
            switch (key) {
