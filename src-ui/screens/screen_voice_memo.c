...
/* Playback speed */
static int playback_speed = 100; /* percentage: 100 = normal */
...
static void draw_playback(struct ncplane *phone, unsigned rows, unsigned cols) {
    ...
    ghost_text(phone, CONTENT_START_ROW + 5, CONTENT_COL, theme_text_primary(), time_line);

    /* Progress bar */
    int bar_row = CONTENT_START_ROW + 7;
    if (bar_row < footer - 3) {
    ...
    /* Speed indicator */
    char speed_str[32];
    snprintf(speed_str, sizeof(speed_str), "Speed: %d%%", playback_speed);
    ghost_text(phone, CONTENT_START_ROW + 9, CONTENT_COL, theme_text_muted(), speed_str);

    /* Controls hint */
    ghost_text(phone, footer - 1, CONTENT_COL, theme_text_muted(),
               "\u25C0\u25B6:Seek \u25B2\u25BC:Speed");
    ghost_softkeys(phone, "[Back]", "[Delete]");
}
...
static void draw_list(struct ncplane *phone, unsigned rows, unsigned cols) {
    ...
    if (count == 0) {
        ...
        ncplane_putstr_yx(phone, mid + 1, CONTENT_COL, "Press right soft key to record");
    ...
    ghost_text(phone, footer - 1, CONTENT_COL, theme_text_muted(),
               "\u25C0:Del \u25B6:Rename OK:Play");
    ghost_softkeys(phone, "[Back]", "[Record]");
    if (delete_prompt) ghost_confirm_popup(phone, "DELETE MEMO?", delete_yes);
}
...
screen_id screen_voice_memo_input(uint32_t key) {
    ...
    if (ui_mode == VM_MODE_PLAYBACK) {
        ...
        switch (key) {
            case NCKEY_LEFT:
                /* Seek backward */
                /* TODO: implement seek in voice_memo_service */
                return SCREEN_VOICE_MEMO;
            case NCKEY_RIGHT:
                /* Seek forward */
                return SCREEN_VOICE_MEMO;
            case NCKEY_UP:
                /* Speed up */
                if (playback_speed < 200) playback_speed += 25;
                return SCREEN_VOICE_MEMO;
            case NCKEY_DOWN:
                /* Slow down */
                if (playback_speed > 50) playback_speed -= 25;
                return SCREEN_VOICE_MEMO;
            case NCKEY_ENTER:
            case '\n':
                /* Toggle play/pause */
