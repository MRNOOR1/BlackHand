...
static void draw_list(struct ncplane *phone) {
    ...
    if (count == 0) {
        ...
        ghost_text(phone, mid + 1, CONTENT_COL, theme_text_muted(), "Press right soft key to create");
    ...
    int max_visible = footer - list_start - 1;
    ...
    for (int i = 0; i < max_visible; i++) {
        ...
        put_clipped(phone, row, CONTENT_COL + 2, width - 2,
                    notes[idx]->title ? notes[idx]->title : "Untitled");
    }
...
static void draw_view(struct ncplane *phone) {
    ...
    ncplane_set_fg_rgb(phone, theme_text_muted());
    ncplane_set_bg_rgb(phone, theme_bg());
    ncplane_putstr_yx(phone, CONTENT_START_ROW, CONTENT_COL, "VIEW");

    ncplane_set_fg_rgb(phone, theme_text_muted());
    for (int x = 0; x < width && CONTENT_COL + x < (int)cols - 1; x++)
        ncplane_putstr_yx(phone, CONTENT_START_ROW + 1, CONTENT_COL + x, "\u2500");

    int content_start = CONTENT_START_ROW + 2;
...
static void draw_edit(struct ncplane *phone) {
    ...
    /* Content only - no title field */
    int body_row = CONTENT_START_ROW + 2;
...
static void draw_save_prompt(struct ncplane *phone) {
    ...
    ghost_text(phone, top + 1, left + 2, theme_text_primary(), "SAVE NOTE AS:");
    ghost_text(phone, top + 3, left + 2, theme_text_primary(), save_name_buf);

    /* Show cursor */
    int cursor_col = left + 2 + save_name_cursor;
...
screen_id screen_notes_input(uint32_t key) {
        ...
        switch (key) {
            ...
            case '\n': {
                ...
                if (save_name_buf[0] != '\0') {
                    ...
                    /* Rename the most recently touched note (index 0) */
                    if (count > 0 && notes && notes[0]) {
                        notes_service_rename_note(notes[0], save_name_buf);
                    }
                }
                mode = NOTES_MODE_LIST;
                selected = 0;
            ...
            default:
                if ((key >= '0' && key <= '9') || key == '*') {
                    /* If buffer still has the default name and this is first key press,
                     * clear it so user can type fresh */
    ...
    if (mode == NOTES_MODE_LIST) {
        ...
        switch (key) {
            ...
            case NCKEY_ENTER:
            case '\n':
                /* Center key = view in read-only mode */
                if (count > 0) {
                    scroll_offset = 0;
                ...
                return SCREEN_NOTES;
            case NCKEY_RIGHT:
                /* Right arrow = edit the note */
                if (count > 0) {
                    begin_edit_existing(selected);
                }
                return SCREEN_NOTES;
            case NCKEY_LEFT:
                /* Left arrow = delete prompt */
                if (count > 0) {
                    delete_prompt = 1;
                    delete_choice_yes = 0;
