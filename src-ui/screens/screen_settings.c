...
static void draw_main(struct ncplane *phone) {
    ...
    for (int i = 0; i < MAIN_ITEM_COUNT; i++) {
        ...
        ncplane_putstr_yx(phone, row, CONTENT_COL + 2, main_items[i]);
    }
...
static void draw_appearance(struct ncplane *phone) {
    ...
    int total = theme_service_is_dark() ? 1 : 2; /* dark mode has no theme option */
    ...
    if (row < footer) {
        ncplane_set_fg_rgb(phone, (s_sub_selected == 0) ? theme_text_primary() : theme_text_muted());
        ncplane_set_bg_rgb(phone, theme_bg());
        ...
        ncplane_putstr_yx(phone, row, CONTENT_COL + 2,
                          settings_service_get_bool("night_mode") ?
                          "[ON]  DARK MODE" : "[OFF] DARK MODE");
    }

    /* Theme option (only in light mode) */
    if (!theme_service_is_dark()) {
        row = CONTENT_START_ROW + 4;
        if (row < footer) {
            ncplane_set_fg_rgb(phone, (s_sub_selected == 1) ? theme_text_primary() : theme_text_muted());
            ncplane_set_bg_rgb(phone, theme_bg());
            ncplane_putstr_yx(phone, row, CONTENT_COL,
                              (s_sub_selected == 1) ? MENU_CURSOR : MENU_CURSOR_BLANK);
            ncplane_putstr_yx(phone, row, CONTENT_COL + 2, "[OPEN] THEMES");
        }
    }
...
static void draw_security(struct ncplane *phone) {
    ...
    for (int i = 0; i < total; i++) {
        ...
        ncplane_set_fg_rgb(phone, (i == s_sub_selected) ? theme_text_primary() : theme_text_muted());
        ncplane_set_bg_rgb(phone, theme_bg());
        ncplane_putstr_yx(phone, row, CONTENT_COL,
                          (i == s_sub_selected) ? MENU_CURSOR : MENU_CURSOR_BLANK);
        ncplane_putstr_yx(phone, row, CONTENT_COL + 2, sec_items[i]);
    }
...
static void draw_connectivity(struct ncplane *phone) {
    ...
    ncplane_putstr_yx(phone, row, CONTENT_COL + 2,
                      settings_service_get_bool("bluetooth") ?
                      "[ON]  BLUETOOTH" : "[OFF] BLUETOOTH");
...
static void draw_pin_entry(struct ncplane *phone) {
    ...
    ghost_text(phone, top + 1, left + 2, theme_text_primary(), title);

    /* Show dots for entered digits */
    char dots[8] = "    ";
    for (int i = 0; i < pin_len && i < 4; i++) dots[i] = '*';
    ghost_text(phone, top + 3, left + 10, theme_text_primary(), dots);

    if (pin_error[0] != '\0') {
        ghost_text(phone, top + 5, left + 2, theme_border(), pin_error);
    }
...
screen_id screen_settings_input(uint32_t key) {
    ...
    if (mode == SETT_MODE_APPEARANCE) {
        int total = theme_service_is_dark() ? 1 : 2;
        switch (key) {
            case NCKEY_UP:
            ...
            case 'E':
                if (s_sub_selected == APP_NIGHT_MODE) {
                    ...
                    theme_service_sync_from_settings();
                    if (s_sub_selected >= (theme_service_is_dark() ? 1 : 2))
                        s_sub_selected = 0;
