...
screen_id screen_voice_memo_input(uint32_t key) {
    ...
    switch (key) {
        ...
        case KEY_ACTION_PRIMARY:
            if (count > 0 && list) {
                /* Only transition to playback view if the service accepted
                   the request. If the daemon couldn't open the file or
                   the ALSA device, leave the user on the list rather than
                   showing a stuck "PAUSED" screen (which is what VM_IDLE
                   renders as inside the playback view). */
                if (voice_memo_service_play_start(list[selected]->filename) == 0) {
        ...
        case KEY_SOFT_RIGHT_ACTION:
            s_discard_prompt = 0;
            if (voice_memo_service_record_start() == 0) {
                ui_mode = VM_MODE_RECORDING;
            }
            return SCREEN_VOICE_MEMO;
        case NCKEY_RIGHT:
            if (count > 0) start_rename_prompt();
            return SCREEN_VOICE_MEMO;
        case KEY_SOFT_LEFT_ACTION:
            delete_prompt = 0;
            return SCREEN_HOME;
